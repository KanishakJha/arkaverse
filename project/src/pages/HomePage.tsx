  ...
  useEffect(() => {
    async function loadDashboardData() {
      try {
        ...
        // Get all published and active books
        ...
        if (booksData) {
          // Map structural chapters count concurrently
  ...
  return (
    <div className="min-h-screen bg-zinc-950 text-white pb-24">
      ...
      <div className="max-w-md mx-auto px-4 mt-4 space-y-4">
        ...
        <div className="flex items-center gap-2 overflow-x-auto no-scrollbar pb-2 pr-4 scroll-smooth">
          {genresList.map((genreName) => (
            ...
            >
              {genreName === 'All' && <Compass className="w-3.5 h-3.5" />}
              {genreName}
            </button>
          ))}
        </div>
        ...
        {isLoading ? (
          ...
          <div className="border-2 border-dashed border-zinc-900 rounded-3xl p-10 text-center space-y-2 mt-4 bg-zinc-900/10">
            ...
            <p className="text-zinc-600 text-xs max-w-xs mx-auto">Iss genre context query ke andar abhi koi manuscript uploaded nahi hai.</p>
          </div>
        ) : (
            ...
            {filteredBooks.map((book) => (
              ...
              >
                ...
                <div className="w-24 h-32 rounded-xl overflow-hidden shrink-0 border border-zinc-900 bg-zinc-950 relative shadow-md">
                  ...
                  <span className="absolute bottom-1.5 left-1.5 bg-black/80 backdrop-blur px-1.5 py-0.5 rounded text-[8px] font-black tracking-wider text-emerald-400 border border-emerald-500/10 uppercase truncate max-w-[80px]">
                    {book.genre}
                  </span>
                </div>
                ...
                <div className="flex-1 flex flex-col justify-between min-w-0">
                  <div className="space-y-1">
                    ...
                    <p className="text-zinc-400 text-xs line-clamp-2 mt-1 leading-normal font-medium pr-1 text-zinc-400/80">
                      {book.description || 'No summary parameters provided for this manuscript.'}
                    </p>
                  </div>
